...
public sealed class EntryStrategyApiIntegrationTests
{
    ...
    public async Task HistoryIsScopedNewestFirstAndEmptyHistoryIsReadable()
    {
        ...
        var b = Guid.NewGuid(); var other = ApiEvaluationFixture.Create(b, DispositionReasonCode.EntryCandidate, true);
        var emptyHolding = Guid.NewGuid(); var repo = new InMemoryEvaluationRepository(older, newer, other); var holdings = new FakeHoldingRepository(false, a, b, emptyHolding);
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode); Assert.Equal(2, rows!.Length); Assert.Equal(newer.Evaluation.EntryStrategyEvaluationId, rows[0].GetProperty("entryStrategyEvaluationId").GetGuid()); Assert.Equal(older.Evaluation.EntryStrategyEvaluationId, rows[1].GetProperty("entryStrategyEvaluationId").GetGuid()); Assert.All(rows, row => Assert.Equal(a, row.GetProperty("holdingId").GetGuid())); Assert.DoesNotContain(rows, row => row.GetProperty("entryStrategyEvaluationId").GetGuid() == other.Evaluation.EntryStrategyEvaluationId);
        Assert.Equal(JsonValueKind.String, rows[0].GetProperty("ccosStatus").ValueKind); Assert.Equal(85, rows[0].GetProperty("ccosScore").GetDouble()); Assert.Equal("Strong", rows[0].GetProperty("ccosClassification").GetString()); Assert.False(rows[0].GetProperty("entryCandidateExists").GetBoolean()); Assert.Equal(JsonValueKind.String, rows[0].GetProperty("dispositionReason").ValueKind); Assert.Equal("3.0.0", rows[0].GetProperty("indicatorCalculationVersion").GetString()); Assert.Equal(1, rows[0].GetProperty("configurationVersion").GetInt32()); Assert.Equal("4.0.0", rows[0].GetProperty("strategyVersion").GetString()); Assert.Equal(older.Evaluation.PreferredInitialOptionSymbol, rows[1].GetProperty("preferredInitialOptionSymbol").GetString()); Assert.Equal(older.Evaluation.PreferredInitialStrike, rows[1].GetProperty("preferredInitialStrike").GetDecimal());
