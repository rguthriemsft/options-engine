...
public sealed class SqliteMarketDataCache(OptionsEngineDbContext db) : IMarketDataCache
{
    ...
    public async Task<MarketDataCacheEntry<IReadOnlyList<DateOnly>>?> GetExpirationsAsync(string symbol, string provider, CancellationToken cancellationToken = default) { var value = await db.OptionExpirationCaches.AsNoTracking().Where(x => x.Symbol == symbol && x.Provider == provider).OrderByDescending(x => x.OptionExpirationCacheId).FirstOrDefaultAsync(cancellationToken); return value is null ? null : new(JsonSerializer.Deserialize<DateOnly[]>(value.ExpirationsJson) ?? [], value.RetrievedAt); }
    ...
    public async Task SaveOptionChainAsync(OptionChain chain, CancellationToken cancellationToken = default)
    {
        if (chain.Contracts.Count == 0)
            db.EmptyOptionChainSnapshots.Add(new EmptyOptionChainSnapshotEntity { UnderlyingSymbol = chain.UnderlyingSymbol,
                Provider = chain.Provider, Expiration = chain.Expiration, Timestamp = chain.Timestamp, TimestampUtcTicks = chain.Timestamp.UtcTicks });
        else
            db.OptionContractSnapshots.AddRange(chain.Contracts.Select(x => new OptionContractSnapshotEntity { OptionSymbol = x.OptionSymbol, UnderlyingSymbol = x.UnderlyingSymbol, Provider = x.Provider, Timestamp = x.Timestamp, TimestampUtcTicks = x.Timestamp.UtcTicks, Expiration = x.Expiration, Strike = x.Strike, OptionType = x.OptionType, Bid = x.Bid, Ask = x.Ask, Last = x.Last, Volume = x.Volume, OpenInterest = x.OpenInterest, ImpliedVolatility = x.ImpliedVolatility, Delta = x.Delta, Gamma = x.Gamma, Theta = x.Theta, Vega = x.Vega, UnderlyingPrice = x.UnderlyingPrice }));
