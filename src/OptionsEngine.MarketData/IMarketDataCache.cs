...
public interface IMarketDataCache
{
    ...
    Task<HistoricalBarsCacheEntry?> GetHistoricalBarsAsync(string symbol, DateOnly start, DateOnly end, string provider, CancellationToken cancellationToken = default); Task UpsertHistoricalBarsAsync(IReadOnlyList<HistoricalBar> bars, DateOnly requestedStart, DateOnly requestedEnd, DateTimeOffset retrievedAt, CancellationToken cancellationToken = default);
...
public sealed record HistoricalBarsCacheEntry(IReadOnlyList<HistoricalBar> Value, DateOnly CoverageStart, DateOnly CoverageEnd, DateTimeOffset RetrievedAt);
